...
void Camera::Update()
{
	...
	else	// FPS Camera Mode
	{
		...
		if( Input::Instance()->GetKeyboard(Spacebar) )			mTrf.Position += mTrf.Up()      * Timer::Instance()->DeltaTime() * mSpeed * mSpeedMultiplier;
		if( Input::Instance()->GetKeyboard(LeftCtrl) )			mTrf.Position -= mTrf.Up()      * Timer::Instance()->DeltaTime() * mSpeed * mSpeedMultiplier;
