...
DX11Wrapper::DX11Wrapper()
{
	...
	XMStoreFloat4x4(&mDragonWorld, XMMatrixMultiply(dragonScale, dragonOffset));
...
void DX11Wrapper::BuildGeometryBuffers()
{
	...
	vbd.ByteWidth = (UINT) (sizeof(Vertex) * totalVertexCount);
...
void DX11Wrapper::DrawScene()
{
	...
	mfxWorldViewProj->SetMatrix(reinterpret_cast<float*>(&viewProj));
