...
public:
	...
	StructuredBuffer(ID3D11Device* d3dDevice, int elements,
					 u32 bindFlags = D3D11_BIND_UNORDERED_ACCESS | D3D11_BIND_SHADER_RESOURCE,
					 bool dynamic = false);
...
{
	if (mUnorderedAccess) RJE_SAFE_RELEASE(mUnorderedAccess);
	if (mShaderResource)  RJE_SAFE_RELEASE(mShaderResource);
	RJE_SAFE_RELEASE(mBuffer);
}
